...
namespace entregable
{
    public partial class frmEntradasInventario : Form
    {
        ...
        private void GuardarEntradaInventario()
        {
            ...
            string query = "INSERT INTO EntradasInventario (ProductoId, Cantidad, PrecioCompra, Fecha) VALUES (@productoId, @cantidad, @precioCompra, @fecha)";
            SqlCommand cmd = new SqlCommand(query, con);
            ...
            try // Wrap database operations in a try-catch block for error handling
            {
                ...
                MessageBox.Show("Entrada de inventario guardada correctamente");
                LimpiarFormulario();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error guardando entrada de inventario: " + ex.Message);
            }
        }
